namespace Runner
{
    public class Program
    {
        ...
        public static int DoSomething(string[] cliArgs)
        {
            ...
            var secs = random.Next(3, 10);
            System.Threading.Tasks.Task.Delay(TimeSpan.FromSeconds(5)).Wait(); //System.Threading.Tasks.Task.Delay(TimeSpan.FromSeconds(secs)).Wait();
            var exitCode = 0;

            var taskId = cliArgs.FirstOrDefault(a => a.StartsWith("--taskId="));
            if (taskId == null)
            {
